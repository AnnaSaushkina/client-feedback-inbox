...
const withItems = (...items: Task[]): TasksState => ({
  ...initialState,
  items,
});
...
describe("tasksSlice reducer", () => {
  ...
  it("fetchTasks.pending ставит loading: true", () => {
    const state = tasksReducer(
      initialState,
      fetchTasks.pending("req", undefined),
    );
  ...
  it("fetchTasks.fulfilled загружает задачи", () => {
    const state = tasksReducer(
      initialState,
      fetchTasks.fulfilled([task1, task2], "req", undefined),
    );
  ...
  it("addTask.fulfilled type=create добавляет задачу", () => {
    const state = tasksReducer(
      initialState,
      addTask.fulfilled({ type: "create", task: task1 }, "req", task1),
    );
  ...
  it("addTask.fulfilled type=update заменяет существующую задачу", () => {
    ...
    const state = tasksReducer(
      withItems(task1, task2),
      addTask.fulfilled({ type: "update", task: updated }, "req", task1),
    );
    expect(state.items).toHaveLength(2);
  ...
  it("updateTask.fulfilled заменяет задачу по id", () => {
    ...
    const state = tasksReducer(
      withItems(task1, task2),
      updateTask.fulfilled(updated, "req", updated),
    );
  ...
  it("toggleTask.fulfilled меняет задачу на выполненную", () => {
    ...
    const state = tasksReducer(
      withItems(task1),
      toggleTask.fulfilled(completed, "req", "1"),
    );
  ...
  it("restoreFromArchive.fulfilled сбрасывает completed и completedAt", () => {
    ...
    const restored = {
      ...
      status: "в_работе" as const,
    };
    const state = tasksReducer(
      withItems(archived),
      restoreFromArchive.fulfilled(restored, "req", "1"),
    );
