...
const priorityColor: Record<string, string> = { low: "default", high: "red" };
const priorityLabel: Record<string, string> = { low: "⚪ Низкий", high: "🔴 Высокий" };
      ...
      {isEditing ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={fieldStyle}>
            ...
            <TextArea value={description} onChange={(e) => setDescription(e.target.value)} rows={6} style={{ resize: "vertical" }} />
          </div>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            ...
            <div style={{ ...fieldStyle, flex: 1 }}>
              ...
              <Select value={assignee} onChange={setAssignee} placeholder="Кто" options={assigneeOptions} allowClear size="large" />
            </div>
          </div>
              ...
              {task.history.map((entry, i) => (
                <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  <span style={{ fontSize: 11, color: "#555", whiteSpace: "nowrap", marginTop: 3, minWidth: 90 }}>
                    {new Date(entry.date).toLocaleDateString("ru", { day: "2-digit", month: "2-digit" })}
                  </span>
          ...
          {task.screenshots && task.screenshots.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {task.screenshots.map((src, i) => (
                <Image key={i} src={src} style={{ width: 140, height: 140, objectFit: "cover", borderRadius: 6 }} />
              ))}
            </div>
          )}
        </div>
