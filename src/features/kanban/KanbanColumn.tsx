...
import { COLUMN_COLORS, COLUMN_LABELS } from "./kanban.constants";
...
function getColumnStyle(
  ...
  if (validity)
    return VALIDITY_STYLE[validity] as {
      borderColor: string;
      background: string;
    };
  if (isOver)
...
function KanbanColumn({
  ...
  const { borderColor, background } = getColumnStyle(validity, isOver, status);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {validity === "valid" && (
          <Text style={{ fontSize: 11, color: "#52c41a", flexShrink: 0 }}>
            ← сюда
          </Text>
        )}
        {validity === "invalid" && (
          <Text style={{ fontSize: 11, color: "#ff4d4f", flexShrink: 0 }}>
            ✕
          </Text>
        )}
      </div>
